  ...
  return (
    <div className="focus" id="focus" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      <div className="bar">
        ...
        {cur !== 'free'
          ? <button className="btn edit" onClick={() => { onClose(); openDrawer('role', { roleId: cur }); }}>✎ Edit role</button>
          : <button className="btn edit" onClick={() => { onClose(); openDrawer('profile'); }}>✎ Edit profile</button>}
      </div>
      </div>
